...
test('edits require approval, reject stale versions, and archive preserves awarded points', async () => {
  const service = createChoreService(createMemoryStorage(), { now: () => new Date('2026-09-05T12:00:00Z') });
...
test('an occurrence edit leaves future settings intact and stopping a series prevents recurrence', async () => {
  ...
  const service = createChoreService(createMemoryStorage(), { now: () => time });
  const householdId = 'solo'; const memberId = 'solo-member';
  await service.setHouseholdMembers(householdId, [memberId]);
